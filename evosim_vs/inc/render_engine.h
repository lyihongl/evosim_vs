...
void RenderDynamicVertexArray(const VertexArray& dat, const Shader& shader);
	...
	*/
	...
	struct VertexManager {
		...
		void CreateBufferObj(std::string name);
		void CreateBufferObj(std::string name, GLsizeiptr size);
		//GLuint GetElementBuffer(std::string name);
		GLuint GetArrayBuffer(std::string name);
		GLuint GetArrayObject(std::string name);
	};
};
